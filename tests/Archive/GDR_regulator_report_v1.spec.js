...
test('test', async ({ page }) => {
  ...
  await expect(page.getByTestId('Modal')).toMatchAriaSnapshot(`
    ...
    - text: /Please note the current submission record will become Inactive, and the resubmission record will become the Active record for "CL" on position date "\\d+-\\d+-\\d+"\\./
    - button "Apply"
    - button "Cancel"
  `);
  ...
  await expect(page.getByRole('main')).toMatchAriaSnapshot(`
    ...
    - img
    - text: /Upload submission file 3 Preview submission information 4 Submit for preliminary validation check Entity code - Entity name.*DCP Granular Data.*Submission type.*Data area.*Position date.*Attachment\\(s\\)/
    - img
    - link /.+\\.zip/
    - button "Back"
    - button "Submit"
